...
import { HOLE_SHAPES, SHED } from './courseLayout.ts';
import { assertNoRoughOverlap, offsetPoints, polygonPath, polylinePath } from '../engine/geometry.ts';
import { assertHolePlacement } from '../engine/placement.ts';
...
export function holesForCount(count) {
  ...
  if (n >= EXPANDED_HOLE_COUNT) return HOLES.concat(BACK_NINE);
  if (n > HOLE_COUNT) return HOLES.concat(BACK_NINE.slice(0, n - HOLE_COUNT));
  return HOLES;
}
