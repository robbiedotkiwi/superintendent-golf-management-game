...
import { VOLUNTEER_SURFACES } from '../data/config.ts';
import { cashOnHand } from './cash.ts';
import { migrateWorkerTier } from './staffTiers.ts';
import { applyWorkedDayMorale, rollSickDay, tickLowMoraleStreak, volunteerMinutesForDay } from './staffMorale.ts';
import { minutesTodayForWeather } from './weather.ts';
import { playerSiteMinutes, projectWorkerIds } from './projects.ts';
...
function flagWeekPlanJobs(state, workerId) {
  ...
  for (const [day, entry] of Object.entries(days) as [string, any][]) {
...
export function applyEarlyStartComplaints(state) {
  ...
  const silent = Object.values(state.machineUpgrades ?? {}).some((list: any) => (list ?? []).includes('ledLightKit'));
