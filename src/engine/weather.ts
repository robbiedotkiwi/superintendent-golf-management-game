...
import { FORECAST_UNRELIABLE_FROM_DAY } from '../data/config.ts';
...
export function pickWeather(weights, rng, exclude = null) {
  ...
  let entries = (Object.entries(weights) as [string, number][]).filter(([type, weight]) => weight > 0 && !blocked.has(type));
  if (!entries.length) {
    entries = (Object.entries(weights) as [string, number][]).filter(([, weight]) => weight > 0);
  }
