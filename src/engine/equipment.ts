...
import { getUpgrade, upgradeAppliesTo } from '../data/upgrades.ts';
import { getTask, hireCostFor, machineRequirementOf, taskUsesMachine } from '../data/tasks.ts';
import { hocFactor } from './mowing.ts';
import { jobHolesFor, setupMinutesFor, variableJobMinutes } from './jobs.ts';
import { handWaterMinutes } from './moisture.ts';
import { taskTimeMultiplier } from './projects.ts';
import { bumpCapitalSpent } from './history.ts';
import { workerTimeMultiplier, mowingOperatorTimeMultiplier, workerBringsOwnMower } from './skills.ts';
import { hasMechanic } from './staff.ts';
import { createRng } from './rng.ts';
import { formatMoney } from './format.ts';
import { needsCapital, needsCash, spendCapital, spendCash } from './cash.ts';
import { OWN_MOWER_PASS_CLASS, PASS_AREAS, TASK_MACHINE_CLASS_LABELS, TASK_MACHINE_CLASS_MOWER } from '../data/config.ts';
import { supportMinutes } from './support.ts';
import { passMinutesFor } from './passes.ts';
import { snapMinutes } from './duration.ts';
...
function overrideUsable(state, machineId, task, options: any = {}) {
...
export function pickMachine(state, task, options: any = {}) {
  const { ignoreTaskId, minutesNeeded } = options;
