...
import { calendarFromDay } from './calendar.ts';
import { buildForecast, canBuyWeatherStation } from './weather.ts';
import { createRng } from './rng.ts';
import { buyFoley, buyMachine, buyUpgrade, grindInHouse, repairMachine, sendForGrind, machinePlanCheck, durationOnMachine, recomputePlannedMinutes, allowingMachines, pickMachine, pickMachineForTask, machineMinutesRemaining, MACHINE_BOOKED_REASON, NO_MACHINE_REASON, getMachine, normalizeMachineOverride, machineSuitability, canHireForTask, missingMachineReason } from './equipment.ts';
import { machineAllows } from '../data/equipment.ts';
import { machineTitle } from './machineDisplay.ts';
import { assignWorker, certifiedPresent, workerById, workerAllows, workerBringsOwnMower, isWorkerPresent } from './assignment.ts';
import { findPlannedJob, jobHolesFor } from './jobs.ts';
import {
  getDayTasks,
  planningDayOf,
  planViewState,
...
import { rosterWorker } from './weekGrid.ts';
import { migrateWorkerTier } from './staffTiers.ts';
import { applyNamedTemplate, copyYesterday, nextStartMinute, saveNamedTemplate, snapMinutes } from './slots.ts';
import {
  autoMachineFor,
  clampBlockResize,
  defaultBlockMinutes,
} from './dayPlanner.ts';
import { approveLeave, declineLeave } from './staffMorale.ts';
import { coringWindowOk, isDrySpell, sprayWindowOk } from './support.ts';
import {
...
import { emptyDisease, emptyUntil } from './disease.ts';
import {
  allGreenIds,
  canBuyGreensSensors,
  canBuyTurfRad,
...
import { canBuyAerator, IRRIGATED_SURFACES, clampIrrigationMm, migrateIrrigationValue } from './irrigation.ts';
import { leaseMachine, stopLease, takeLoan } from './budget.ts';
import { emptyDaysSinceWorked, markMailRead, meetingDue } from './mail.ts';
import {
  applySnapTournament,
  comingSeason,
...
import { clampStanding } from './satisfaction.ts';
import { buyAutoPicker, pauseProject, resumeProject, startGrassConversion, startProject } from './projects.ts';
import { bumpCapitalSpent, emptyYearRecord } from './history.ts';
import { spendCapital } from './cash.ts';
import { dismissGm, emptySectionUnlocks, GM_MSG_DAY1, isSectionLocked } from './gm.ts';
import { snapshotWeekStart } from './weekReview.ts';
import { resolveDay } from './simulation.ts';
import {
  CUT_TASK_BY_SURFACE,
  DAMAGING_JOB_REASON,
  DAY_LENGTH_MINUTES,
...
import { clampAngle, clampHoc, hasHoc, hasPattern, mergeSurfaceFields, angleDelta } from './mowing.ts';
import { startingGrass } from './grass.ts';
import {
  createInitialHoles,
...
import { courseBounds, holesForCount } from '../data/course.ts';
import { clampView, defaultView } from './view.ts';
import { defaultSectionTabs, normalizeSection, normalizeTabs, tabListForSection } from './section.ts';
import { formatMoney } from './format.ts';
import { CAPEX_STATUS_PENDING, STARTING_CAPEX, STARTING_CASH, STARTING_MONTHLY_BUDGET, TASK_MACHINE_CLASS_MOWER } from '../data/config.ts';
import { buyUsed, rollUsedListings, sellMachine } from './market.ts';
import { acceptEvent, declineEvent } from './events.ts';
...
export function taskWindowCheck(state, taskId, options: any = {}): any {
...
export function canPlanTask(state, taskId, workerId?, options: any = {}): any {
