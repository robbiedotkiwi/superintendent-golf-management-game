...
import { PASS_AREAS, AUTONOMOUS_AREAS, AUTONOMOUS_NIGHT_MINUTES, AUTONOMOUS_UNSTICK_CHANCE, AUTONOMOUS_UNSTICK_MINUTES, CAPEX_STATUS_MISSED, CAPEX_STATUS_RELEASED, MINUTES_PER_HOUR } from '../data/config.ts';
import { applyCoringSeasonTick, applySupportDay, applySupportWeekEnd } from './support.ts';
import { applyDailyQualityDrift, applyMissedWeekPenalties } from './qualityDrift.ts';
import { applyAreaQualityToHoles, applyWeekPasses, emptyAreaQuality, machineAllowsArea, passHoursFor, resolveDayPasses } from './passes.ts';
import { migrateWorkerTier } from './staffTiers.ts';
import { generateCandidates, generateCasuals } from '../data/staff.ts';
import { getTask, hireCostFor, machineRequirementOf, taskAppliesQuality } from '../data/tasks.ts';
import { workerById, workerQualityMultiplier, qualityRandomFactor } from './assignment.ts';
import { workerBringsOwnMower } from './skills.ts';
import { enqueueGm, GM_MSG_CAPEX_MISSED, GM_MSG_CAPEX_RELEASED, GM_MSG_CORING, GM_MSG_DUTIES, tickGm } from './gm.ts';
import { consumeJobFuel, jobBurnsFuel, replaceBurnSpend } from './fuel.ts';
import { applyMowingAftermath, hocStressApplies, mowingGain, rotatePatternAngle } from './mowing.ts';
import {
  cloneHoles,
...
import { calendarFromDay, daysUntilSeasonEnd } from './calendar.ts';
import {
  applyWear,
  applyMachineHours,
  applyConditionLoss,
...
import { applyEarlyStartComplaints, applyMorale, departResignedWorkers, prepareMorningWorkers, wageBill } from './staff.ts';
import { maybeLeaveRequest } from './staffMorale.ts';
import { activateDayPlan, casualsBookedOn, getDayTasks, lockWeek, rollNewWeek, setDayTasks, weekStartDay } from './week.ts';
import { createRng } from './rng.ts';
import { applyFertiliser, applySpray, emptyDisease, resolveDisease, syncHoleDisease } from './disease.ts';
import { resolveIrrigation } from './irrigation.ts';
import {
  applyHandWater,
  droughtDecay,
  emptyMoisture,
...
import { rollMorningWithRng } from './weather.ts';
import { closeSeason, seasonGrant } from './budget.ts';
import { applyMonthlyBudget, applySeasonCapexGrant, tryReleaseSeason1Capex } from './economy.ts';
import { golferMail, gmMissedTournamentMail, gmSeasonMail, gmTournamentRequestMail, grantForecastMail, meetingDue, pushMail, resignMail, tickDaysSinceWorked } from './mail.ts';
import { neglectMail, neglectSatisfactionDrain } from './neglect.ts';
import {
...
import { tickProjects } from './projects.ts';
import { buildYearReview, emptyYearRecord, hiredIds, recordYearDay } from './history.ts';
import { clampRange, clampStanding, tickSatisfaction } from './satisfaction.ts';
import { GM_MEETING_SKIP_STANDING } from '../data/constants.ts';
import { tickMarket, rollUsedListings } from './market.ts';
import { tickEvents } from './events.ts';
import { jobHolesFor, snapshotDayJobs } from './jobs.ts';
import { buildWeekReview, snapshotWeekStart, weekSummariesOf } from './weekReview.ts';
...
export function resolveDay(state) {
  ...
  for (const [surface, amount] of Object.entries(extraDecay) as [string, number][]) {
