...
import { BASE_MINUTES } from './courseArea.ts';
import { hocRangeFor, thatchWearMult, wearRecoveryMult } from './grass.ts';
...
export function mergeSurfaceFields(kind, surface: any = {}, state) {
...
export function applyMowingAftermath(surface, kind, day, wearIncremented, settings: any = {}, state?) {
  const next = { ...surface };
