...
import { getMachine, machineClass } from '../data/equipment.ts';
import { getTask } from '../data/tasks.ts';
import { WEAR_MAX } from '../data/constants.ts';
import { applyDurationModifier, hoursToMinutes, minutesToHours, roundDurationHours } from './duration.ts';
import { clampQuality, gradeCapScore, gradeLetter } from './grades.ts';
import { staffCanRunClass, staffPassTimeMult } from './staffTiers.ts';
import { mapHoleSurfaces } from './holes.ts';
...
export function passHoursFor(state, machineId, area, worker, options: any = {}) {
...
export function passMinutesFor(state, machineId, area, worker, options: any = {}) {
...
export function passJobsFromPlanned(state, planned, workers = []) {
  ...
  for (const job of planned ?? []) {
    ...
    const options: any = { ignoreStaff: autonomous };
