import { PLAYER_ID } from '../data/constants.ts';
import { TASK_MACHINE_CLASS_MOWER } from '../data/config.ts';
...
import { defaultJobHoles } from './holes.ts';
import { migrateIrrigationValue } from './irrigation.ts';
import { daysSinceLastWorked } from './neglect.ts';
import { workerAllows, workerBringsOwnMower } from './skills.ts';
import {
  dropInvalidDayTasks,
  dayLengthMinutes,
  getDayTasks,
...
export function machineCapacityForDay(state, day, plannerId) {
  ...
  const minutesByMachine: Record<string, number> = {};
