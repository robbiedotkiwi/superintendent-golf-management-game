...
import { SEASON_WEEKS } from '../data/config.ts';
import { WEATHER_LABELS } from '../data/events.ts';
import { qualityColor } from '../engine/color.ts';
import { sectionBadge } from '../engine/badges.ts';
import { weekOfSeason } from '../engine/calendar.ts';
import { formatMoney } from '../engine/format.ts';
import { GM_LOCK_HINT, isSectionLocked } from '../engine/gm.ts';
import { pondDoseBriefing } from '../engine/irrigation.ts';
import { getTask } from '../data/tasks.ts';
import { formatTempRange, isHotterThanCall } from '../engine/weather.ts';
import { fitCourse } from '../engine/view.ts';
import { planningDayOf, weekdayLabel } from '../engine/week.ts';
...
function NavButton({ label, description, active, badge, dot, locked = false, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
