...
import { grassSpeciesFor, hocRangeFor } from '../engine/grass.js';
        ...
        return (
          <section key={surface} className="mt-3 border border-[var(--sand)] p-2">
            ...
            <p className="text-sm text-[var(--sand)]">
              {species ? `${species.name} · ` : ''}Quality {Math.round(record.quality)} · {daysSinceHoleWorked(state, holeId, surface)}d since worked
            </p>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      onSetOverride(holeId, surface, {
                        ...settings,
                        hoc: clampHoc(surface, Number(event.target.value), state),
                      })
                    }
