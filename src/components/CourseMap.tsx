...
import { boundaryFill, greenOutline, surfaceFill } from '../engine/color.ts';
import {
  AERATOR_ARM,
  AERATOR_RX,
  AERATOR_RY,
...
import { pondCapacity, pondPercent } from '../engine/irrigation.ts';
import { moistureOverlayColor, moistureStatus, outOfBand } from '../engine/moisture.ts';
import { patternRotate, patternStripeColor, showsMowOverlay, surfacePatternOpacity } from '../engine/pattern.ts';
import { prefersReducedMotion } from '../engine/sound.ts';
import {
  clientToWorld,
  clampView,
  defaultView,
...
function MoistureOverlayShape({ kind, d = undefined, cx = undefined, cy = undefined, rx = undefined, ry = undefined, x = undefined, y = undefined, width = undefined, height = undefined, surface, state }) {
    ...
    >
      ...
      {showMower && !prefersReducedMotion() ? (
        <circle
          ...
          r="7"
          fill="var(--machine-orange)"
          style={{ ['--mower-ms']: `${MOWER_ANIM_MS}ms`, offsetPath: `path('${mowerPathFor(layout)}')` } as any}
        />
      ) : null}
      <text
