import { START_DAY_LABEL } from '../data/constants.ts';
import { SURFACE_LABELS } from '../data/tasks.ts';
import { skippedOverdueSurfaces, unusedTimeCopy } from '../engine/badges.ts';
import { formatMoney } from '../engine/format.ts';
import { plannedDayFuel } from '../engine/fuel.ts';
import { combinedMinutesRemaining } from '../engine/gameState.ts';
import { IRRIGATED_SURFACES, pondDoseBriefing } from '../engine/irrigation.ts';
import { weekdayLabel } from '../engine/week.ts';
import ForecastStrip from './ForecastStrip.tsx';
import IrrigationMmSlider from './IrrigationMmSlider.tsx';
import { WeatherStationReadout } from './WeatherStation.tsx';
...
export default function StartDayDialog({
  ...
  const weekday = weekdayLabel(state.day);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--soil)]/85 p-4">
      <section className="max-h-[90vh] w-full max-w-xl overflow-y-auto border-4 border-[var(--sand)] bg-[var(--soil)] p-6 text-[var(--paint)]">
        <h2 className="font-condensed text-4xl font-bold" data-start-day-heading>
          Start {weekday}
        </h2>
        {pondBriefing ? (
        ...
        <ForecastStrip state={state} selectedDay={state.day} />
