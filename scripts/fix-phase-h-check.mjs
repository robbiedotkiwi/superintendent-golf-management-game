...
const sensors = reducer(createInitialState(), { type: 'BUY_GREENS_SENSORS' });
assert.equal(sensors.hasGreensSensors, true);
assert.equal(sensors.capitalBudget, createInitialState().capitalBudget - GREENS_SENSORS_COST);
...
let rad = reducer(createInitialState(), { type: 'BUY_TURFRAD' });
assert.equal(rad.hasTurfRad, true);
assert.equal(rad.capitalBudget, createInitialState().capitalBudget - TURFRAD_COST);
...
let live = reducer(createInitialState(), { type: 'BUY_GREENS_SENSORS' });
