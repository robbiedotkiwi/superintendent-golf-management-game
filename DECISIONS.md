...
- Task panel (and the pond panel) is `absolute` inside the map pane so it cannot shift layout or cover End day. It unmounts when closed so it cannot leave a soil slab over the map. Machine copy only for mowing and roll; cups and hand water stay silent. Quality buttons lead with minutes.
