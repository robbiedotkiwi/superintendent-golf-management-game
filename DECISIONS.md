...
- Course routing is a walkable 1→9 loop: each next tee sits by the previous green, hole 9 returns to the shed on the south boundary. The property line is a convex hull of the roughs and shed, expanded by `BOUNDARY_EXPAND`, filled darker than the rough. Flags sit on the north edge of each green; tee markers sit northwest of each tee. The range bbox stays in the camera even before the range exists so the view does not jump.
